...
export default function SurveyEditor() {
  ...
  async function loadSurvey() {
    ...
    setIconSize(data.icon_size != null ? data.icon_size : 32)
    setLoading(false)
  }
  ...
  function updateIconSize(s) { const v = Math.max(10, Math.min(80, s)); setIconSize(v); saveSurvey(id, { icon_size: v }) }
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 12px', background: '#f8f8f6', borderBottom: '0.5px solid #e0dfd8', flexShrink: 0, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <input type="range" min="10" max="80" step="2" value={iconSize}
            onChange={e => { const v = parseInt(e.target.value); setIconSize(v); }}
            onMouseUp={e => updateIconSize(parseInt(e.target.value))}
            onTouchEnd={e => updateIconSize(parseInt(e.target.value))}
